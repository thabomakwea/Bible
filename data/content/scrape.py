...
for a in range(1, chapters):
    response = requests.get(
        'https://www.biblegateway.com/passage/?search=Revelation+' + str(a) + '&version=KJ21')
    html = response.text
    ...
    for b, tag in enumerate(soup.find_all("p")):
        ...
        if(str(tag.find_next("span").get("id")) != "None"):
            r += '"Revelation2coOAF2dByWwsL1S' + str(a) + str(b) + '": {'
            r += '"chapter": ' + '"' + str(a) + '",'
            r += '"book": ' + '"' + book + '",'
...
# for a in range(1, chapters):
#     response = requests.get(
#         'https://www.biblegateway.com/passage/?search=1-Kings+' + str(a) + '&version=KJ21')
#     html = response.text
#     soup = BeautifulSoup(html, "html.parser")
